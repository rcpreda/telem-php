<x-app-layout>
    <x-slot name="toolbar">
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <li class="breadcrumb-item text-muted">{{ __('roles.roles') }}</li>
            </ul>
        </div>
    </x-slot>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('roles.manage_roles') }}</h3>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="{{ \App\DataTables\RoleDataTable::TABLE_ID }}" class="table table-bordered align-middle table-striped no-footer dataTable">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th>{{ __('roles.role_name') }}</th>
                            <th>{{ __('roles.permissions') }}</th>
                            <th>{{ __('roles.users') }}</th>
                            <th>{{ __('roles.created_date') }}</th>
                            <th class="text-end pe-5 no-sort">{{ __('roles.actions') }}</th>
                        </tr>
                    </thead>
    ...
    <script>
        $(function() {
            ...
            let table = $('#{{ \App\DataTables\RoleDataTable::TABLE_ID }}').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
